...
{
    public sealed class CachedSIMDSFMT : SFMTBase
    {
        ...
        public void MoveNext()
        {
            ...
            {
                var _s = _head - N32 * 2;
                if (_s < 0) _s = _capacity - N32;

                if (Avx2.IsSupported)
                    SIMDSFMTFunction.GenerateRandAll_Avx2(_cache.AsSpan().Slice(_s, N32), _cache.AsSpan().Slice(_head - N32, N32));
                else if (Sse2.IsSupported)
                    SIMDSFMTFunction.GenerateRandAll_Sse2(_cache.AsSpan().Slice(_s, N32), _cache.AsSpan().Slice(_head - N32, N32));
                else
                {
                    GenerateRandAll();
        ...
        public CachedSIMDSFMT(uint initialSeed, int cacheSegments) : base(initialSeed)
        {
            ...
            // 最初のセグメントは_stateVectorから取ってくる
            if (Avx2.IsSupported)
                SIMDSFMTFunction.GenerateRandAll_Avx2(_stateVector, _cache);
            else if (Sse2.IsSupported)
                SIMDSFMTFunction.GenerateRandAll_Sse2(_stateVector, _cache);
            else
            {
                GenerateRandAll();
                Array.Copy(_stateVector, 0, _cache, 0, N32);
            }

            for (int i = 1; i < _cacheSegments; i++)
            {
                if (Avx2.IsSupported)
                    SIMDSFMTFunction.GenerateRandAll_Avx2(_cache.AsSpan().Slice(N32 * (i - 1), N32), _cache.AsSpan().Slice(N32 * i, N32));
                else if (Sse2.IsSupported)
                    SIMDSFMTFunction.GenerateRandAll_Sse2(_cache.AsSpan().Slice(N32 * (i - 1), N32), _cache.AsSpan().Slice(N32 * i, N32));
                else
                {
                    GenerateRandAll();
